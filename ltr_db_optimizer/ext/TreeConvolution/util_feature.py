...
def _flatten(root, transformer, left_child, right_child, query_encoding):
    ...
    def recurse(x):
        if _is_leaf(x, left_child, right_child):
            accum.append(np.concatenate((transformer(x),query_encoding.detach().cpu().numpy())))
            return
...
def _flatten_plans_only(root, transformer, left_child, right_child):
    """ turns a tree into a flattened vector, preorder """
...
def _preorder_indexes(root, left_child, right_child, idx=1):
    ...
    left_subtree = _preorder_indexes(left_child(root), left_child, right_child, idx=idx+1)
    
    max_index_in_left = rightmost(left_subtree)
    right_subtree = _preorder_indexes(right_child(root), left_child, right_child, idx=max_index_in_left + 1)
...
def prepare_trees(trees, transformer, left_child, right_child, query_enc, cuda=False):
    flat_trees = [_flatten(x, transformer, left_child, right_child, query_enc[idx]) for idx, x in enumerate(trees)]
...
def prepare_trees_plans_only(trees, transformer, left_child, right_child,cuda=False):
    flat_trees = [_flatten_plans_only(x, transformer, left_child, right_child) for x in trees]
    flat_trees = _pad_and_combine(flat_trees)
    flat_trees = torch.Tensor(flat_trees)
